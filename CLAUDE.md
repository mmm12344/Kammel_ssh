...
### Localization (ES/EN/ZH)

Spanish is the source language and the lookup key: every user-facing string is written in Spanish inside `tr('…')` (`lib/l10n/l10n.dart`), and each `strings_*.dart` table maps that exact Spanish text to its language — `strings_en.dart` (English) and `strings_zh.dart` (Simplified Chinese; `AppLang` is `es/en/zh`). A key with no entry falls back to the Spanish text, so a partial translation is always safe.
...
- `python3 scripts/i18n_check.py` lists `tr()` keys missing from **every** `strings_*.dart` table it auto-discovers (en *and* zh; `--stubs` prints pasteable lines) — CI fails on a missing entry. `test/l10n_test.dart` checks fallback, placeholder substitution, and that no translation drops a placeholder; `test/changelog_test.dart` additionally pins the Chinese changelog entries.
- **`tr()` keys are load-bearing text, not identifiers**: fixing a typo in a Spanish source string means updating the key in en *and* zh in the same commit, or that string silently falls back to Spanish (or the old wording) for everyone else.
...
The UI is a hand-rolled "Swiss / flat IDE" system (`widgets/swiss.dart` provides `SwissPanel`, `Hairline`, `ContentColumn`, `ToggleRow`, `InvertedButton`, `GhostButton`, `MonoTag`…), built on three **mutable static palettes** in `theme/app_theme.dart`: DARKROOM (dark — ink `0xFF0A0A0A`, bone `0xFFECE7DD`, hairline `0xFF2A2A2A`), PAPER (light) and OLED (true black), switched at runtime together with an **accent** (7 presets + user-picked, stored under `settings_accent_color_hex`). `BorderRadius.zero` is the look — square corners everywhere — and hairlines (1px) are the borders. Type is Anton (display wordmarks) / Archivo (labels & body) / JetBrains Mono (mono), with small **uppercase letter-spaced labels** (`AppText.label`). The one hue allowed to compete with content is a profile's signal color (see `profileTint`), drawn as a stripe or dot, never a fill. Terminal schemes (`warp`, `dracula`, `gruvbox`, `nord`, `oled`, `paper`) live in the same file and recolor cursor/selection to the accent. Match these constants rather than default Material styling when adding UI. UI copy is authored in Spanish and wrapped in `tr()` — see Localization above.
